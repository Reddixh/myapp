...
<head>
    ...
    <title>Student Detail — IT Department</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-return { background: #dcfce7; color: #166534; border: 1px solid #86efac; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; cursor: pointer; }
        .btn-lost { background: #fee2e2; color: #991b1b; border: 1px solid #fca5a5; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; cursor: pointer; }
        ...
        .equip-row { display: flex; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid #f1f5f9; }
        .equip-row:last-child { border-bottom: none; }
        .equip-icon { width: 40px; height: 40px; border-radius: 10px; background: #E6F1FB; display: flex; align-items: center; justify-content: center; font-size: 20px; flex-shrink: 0; }
        .equip-info { flex: 1; }
        .equip-name { font-size: 13px; font-weight: 600; color: #1e293b; }
        .equip-meta { font-size: 11px; color: #64748b; margin-top: 2px; }
...
<div class="content">
    <a href="{{ route('it.dashboard') }}" class="back-link">← Back to Dashboard</a>
    ...
    @php
        $hasEquipment = $equipment->where('status','issued')->count() > 0;
        $hasPenalties = $penalties->where('status','unpaid')->count() > 0;
        $canAutoClear = !$hasEquipment && !$hasPenalties && $clearance && $clearance->status === 'pending';
    @endphp

    @if($canAutoClear)
        <div class="auto-clear-box">
            🤖 <strong>Auto-clearance eligible!</strong> This student has no issued equipment and no unpaid penalties.
        </div>
    @endif
    ...
    <div class="grid-2">
        ...
        <div>
            ...
            <div class="card">
                ...
                <div class="action-grid">
                    <form method="POST" action="{{ route('it.approve', $student->id) }}">
                        @csrf
            ...
            <div class="card">
                ...
                <div style="margin-top:16px;padding-top:14px;border-top:1px solid #f1f5f9">
                    ...
                    <form method="POST" action="{{ route('it.penalty', $student->id) }}">
                        @csrf
                        <div class="form-group">
                            <label class="form-label">Name</label>
                            <input type="text" name="name" class="form-input" placeholder="e.g. Lost equipment fine" required>
                        </div>
        ...
        <div>
            <div class="card">
                <div class="card-title">IT Equipment ({{ $equipment->count() }})</div>
                @forelse($equipment as $item)
                <div class="equip-row">
                    <div class="equip-icon">💻</div>
                    <div class="equip-info">
                        <div class="equip-name">{{ $item->equipment_name }}</div>
                        <div class="equip-meta">
                            {{ $item->category }}
                            @if($item->serial_number) | SN: {{ $item->serial_number }} @endif
                        </div>
                        <div class="equip-meta">
                            Issued: {{ $item->issued_date->format('d M Y') }}
                    ...
                    @if($item->status === 'issued')
                        <div style="display:flex;flex-direction:column;gap:4px">
                            <form method="POST" action="{{ route('it.equipment.return', $item->id) }}">
                                @csrf
                                <button type="submit" class="btn-return">Return</button>
                            </form>
                            <form method="POST" action="{{ route('it.equipment.lost', $item->id) }}">
                                @csrf
                                <button type="submit" class="btn-lost">Lost</button>
                            </form>
                        </div>
                    @endif
                </div>
                @empty
                    <div style="text-align:center;padding:16px;color:#94a3b8;font-size:13px">No equipment issued.</div>
                @endforelse

                <div style="margin-top:16px;padding-top:14px;border-top:1px solid #f1f5f9">
                    <div class="card-title">Issue New Equipment</div>
                    <form method="POST" action="{{ route('it.equipment.add', $student->id) }}">
                        @csrf
                        <div class="form-group">
                            <label class="form-label">Equipment Name</label>
                            <input type="text" name="equipment_name" class="form-input" placeholder="e.g. Dell Laptop" required>
                        </div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
                            <div class="form-group">
                                ...
                                <select name="category" class="form-input">
                                    ...
                                    <option>Network Card</option>
                                    <option>Other</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Serial Number</label>
                                <input type="text" name="serial_number" class="form-input" placeholder="SN-12345">
                            </div>
                        </div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
                            <div class="form-group">
                                <label class="form-label">Issued Date</label>
                                <input type="date" name="issued_date" class="form-input" value="{{ date('Y-m-d') }}" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Due Date</label>
                                <input type="date" name="due_date" class="form-input">
                        ...
                        <button type="submit" class="btn-submit">💻 Issue Equipment</button>
                    </form>
                </div>
            </div>
        </div>
...
<div class="modal-overlay" id="rejectModal">
    <div class="modal">
        ...
        <form method="POST" action="{{ route('it.reject', $student->id) }}">
            @csrf
            <div class="form-group">
                <label class="form-label">Reason for rejection</label>
                <textarea name="remarks" class="form-input" rows="3" placeholder="e.g. Student has unreturned equipment..." required></textarea>
            </div>
