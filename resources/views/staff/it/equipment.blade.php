...
<head>
    ...
    <title>All Equipment — IT Department</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card { background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px 20px; }
        ...
        .status-issued { background: #E6F1FB; color: #0C447C; font-size: 11px; padding: 2px 8px; border-radius: 20px; }
        .status-returned { background: #dcfce7; color: #16a34a; font-size: 11px; padding: 2px 8px; border-radius: 20px; }
        .status-lost { background: #fee2e2; color: #991b1b; font-size: 11px; padding: 2px 8px; border-radius: 20px; }
...
<div class="topbar">
    <div>
        ...
        <div class="topbar-sub">All Equipment</div>
    </div>
...
<div class="sidebar">
    ...
    <a href="{{ route('it.dashboard') }}" class="nav-item">🏠 Dashboard</a>
    <hr class="nav-divider">
    <div class="nav-section">Equipment</div>
    <a href="{{ route('it.equipment') }}" class="nav-item active">💻 All Equipment</a>
    <hr class="nav-divider">
...
<div class="content">
    ...
    <div class="page-title">All IT Equipment</div>
    <div class="page-sub">Track all equipment issued to students.</div>

    <div class="filter-bar">
        <button class="filter-btn active" onclick="filterEquip('all',this)">All</button>
        <button class="filter-btn" onclick="filterEquip('issued',this)">🔵 Issued</button>
        <button class="filter-btn" onclick="filterEquip('returned',this)">🟢 Returned</button>
        <button class="filter-btn" onclick="filterEquip('lost',this)">🔴 Lost</button>
    </div>
    ...
    <div class="card">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr class="equip-row" data-status="{{ $item->status }}">
                    <td style="font-weight:600">{{ $item->equipment_name }}</td>
                    <td>
                        <div style="font-weight:600">{{ $item->user->name }}</div>
                        <div style="font-size:11px;color:#64748b">{{ $item->user->reg_number }}</div>
                    </td>
                    <td>{{ $item->category }}</td>
                    ...
                    <td>
                        <a href="{{ route('it.student', $item->user->id) }}" class="btn-view">View →</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
        ...
        @else
            <div class="empty-state">💻 No equipment records found.</div>
        @endif
    </div>
</div>

<script>
function filterEquip(status, btn) {
